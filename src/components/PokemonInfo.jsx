...
// api
import { getPokemonByID, getPokemonGif, getSpeciesByID } from "../services/api";
...
const PokemonInfo = ({ id, isShowInfo, setIsShowInfo }) => {
  ...
  const fetchPokemonGif = async () => {
    let res = await getPokemonGif();
    res && setPokemonGif(res);
  };
  ...
  const fetchSpeciesByID = async () => {
    ...
    res.flavor_text_entries.map((data) => {
      if (data.language.name == "en") {
        return setFlavorText(data.flavor_text);
      }
    });
  };
                ...
                {stats.map((stat, index) => {
                  return (
                    <div
                      ...
                      style={{ backgroundColor: `${statsObj[index].color}` }}
                    >
